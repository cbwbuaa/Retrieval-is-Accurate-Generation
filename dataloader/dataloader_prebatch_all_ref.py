...
class CopyisallyouneedWikitext103V2DatasetPrebatchAllRef(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.args['in_doc_neg_num'] > 0:
            min_length = 2
            max_length = 10
            for idx, bert_ids in enumerate(bert_batch):
                if len(bert_ids) - 1 - min_length <= 1:
                    continue
                for _ in range(self.args['in_doc_neg_num']):
                    rand_st = np.random.randint(1, len(bert_ids) - 1 - min_length)
                    rand_end = np.random.randint(rand_st + min_length - 1, min(len(bert_ids) - 1, rand_st + max_length))
                    in_doc_neg_start_labels.append(rand_st + max_bert_length * idx)
                    in_doc_neg_end_labels.append(rand_end + max_bert_length * idx)
