...
# version=0519_asyn_update_lr${lr}_prebatch${prebatch_step}_beta${beta}_warmup${warmup_step}_prenum${prebatch_num}_temp${temp}
version=test
...
gpu_ids=(${cuda//,/ })
CUDA_VISIBLE_DEVICES=$cuda python3 -m torch.distributed.launch --nproc_per_node=${#gpu_ids[@]} --master_addr 127.0.0.1 --master_port 28445 train_asyn.py \
    --dataset $dataset \
    --model $model \
    --multi_gpu $cuda \
