...
class CopyisallyouneedAllRefV2Asyn(nn.Module):

    def __init__(self, **args):
        super(CopyisallyouneedAllRefV2Asyn, self).__init__()
        self.args = args
    ...
    def _update_prebatch_phrase(self, cur_phrase_str, cur_phrase_emb):
        self.pre_batch_phrases.append((cur_phrase_str, cur_phrase_emb.detach()))
        self.pre_batch_phrases = self.pre_batch_phrases[-self.prebatch_step_limit:]

    def _get_prebatch_phrase(self):
        if self.pre_batch_phrases:
            all_str = []
            all_emb = []
            for phrase_str, phrase_emb in self.pre_batch_phrases:
                all_str.extend(phrase_str)
                all_emb.append(phrase_emb)
            all_emb = torch.vstack(all_emb)
            return all_str, all_emb
        else:
            return None, None
    ...
    def _get_random_prebatch_phrase(self):
        ...
        if self.pre_batch_phrases:
            ...
            for phrase_str, phrase_emb in self.pre_batch_phrases:
                ...
                all_emb.append(phrase_emb[random_idx])
            all_emb = torch.vstack(all_emb)
            return all_str, all_emb
        else:
            return None, None
    ...
    def forward(self, batch):  
        ...
        prebatch_str, prebatch_emb = self._get_random_prebatch_phrase()
        # prebatch_str, prebatch_emb = self._get_prebatch_phrase()
        if prebatch_emb is not None:
            candidate_reps = torch.cat([
                candidate_reps,
                prebatch_emb], dim=0)
            valid_tok_suffix = batch['valid_tok_suffix']
            prebatch_false_neg_mask = torch.ones(false_neg_mask.shape[0], len(prebatch_str)).cuda()
            false_neg_idx0 = []
            false_neg_idx1 = []
        ...
        phrase_str = batch['all_phrases']
        self._update_prebatch_phrase(phrase_str, phrase_rep)
