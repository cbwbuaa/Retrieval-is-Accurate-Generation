...
class CopyisallyouneedNeg(nn.Module):

    def __init__(self, **args):
        super(CopyisallyouneedNeg, self).__init__()
        self.args = args
    ...
    ''' 0428 YQC edit: start'''
    def _update_pre_batch_phrase(self, cur_phrase_emb, cur_phrase_text):
        if len(self.pre_batch_phrases) >= self.pre_batch_queue_size:
            self.pre_batch_phrases.pop(0)
        self.pre_batch_phrases.append((cur_phrase_emb, cur_phrase_text))
    ''' 0428 YQC edit: end'''
    ...
    def forward(self, batch):
        ...
        '''TODO: get the phrase text, and then save the emb and text'''
        ...
        try:
            ...
            exit('buggy')
        self._update_pre_batch_phrase(phrase_emb, valid_phrase_texts)
        ''' 0428 YQC edit: end'''
    ...
    def encode_batch(self, sentences, batch_size=256, device='cuda'):
        ...
        for st_idx in range(0, len(sentences), batch_size):
            ...
            output = self.phrase_encoder(**{k: v for k, v in encoded_dict.items() if k != 'offset_mapping'},
                                         output_hidden_states=True)['hidden_states'][-1]  # [B, S, E]
            # collect the phrase start representations and phrase end representations
            s_rep = self.s_proj(output)
            e_rep = self.e_proj(output)
