...
#         print('[!!!] data step1:', os.getpid())
#     train_data, train_iter, sampler = load_dataset(args)
#     while True:
#         for x in train_iter:
#             queue.put(x)

# def data_pipeline(args, model_queue):
#     if args['local_rank'] == 0:
#         print('[!!] data pipeline:', os.getpid())
#     prebatch_queue = mp.Queue(1)
#     step1_generator = mp.Process(target=data_proc, args=(args, prebatch_queue))
#     step1_generator.start()
#     prebatch_processor = load_prebatch_processor(args)
#     while True:
#         batch = prebatch_queue.get()
#         batch = prebatch_processor(batch)
#         model_queue.put(batch)

# def asynchronous_load(args):
#     model_queue = mp.Queue(1)
#     data_generator = mp.Process(target=data_pipeline, args=(args, model_queue))
#     data_generator.start()
#     while True:
#         batch = model_queue.get()
#         yield batch
#     # data_generator.join()
